/* =====================================================
   Página de demandas
   - tela de seleção de plano aparece primeiro
   - o resto do formulário só aparece depois de escolher
   ...
   (function () {
    ...
    const blocoSelecao = document.getElementById("bloco-selecao");
    const areaFormulario = document.getElementById("area-formulario");
    const painel = document.getElementById("painel-formulario");
    const tipoIndefinidoDiv = document.getElementById("tipo-indefinido");
    ...
    const rolar = (el) => el.scrollIntoView({ behavior: movimentoReduzido ? "auto" : "smooth", block: "start" });

    /* ---------- leitura do estado ---------- */
    const planoAtual = () => $('input[name="plano"]:checked', form);
    const tipoChipAtual = () => $('input[name="tipo"]:checked', form);
    const marcados = (nome) => $$(`input[name="${nome}"]:checked`, form);

    function tipoEfetivo() {
        const p = planoAtual();
        if (!p) return "";
        if (p.dataset.tipo) return p.dataset.tipo;      // prata, gold, diamond já têm tipo fixo
        const chip = tipoChipAtual();                    // "ainda não sei" -> depende da escolha
    ...
    function atualizarLimitesEDicas() {
        ...
        const dicaPlat = $("#dica-plataformas");
        if (dicaPlat) {
            dicaPlat.textContent = maxPlat
        ...
        const dicaAdd = $("#dica-adicionais");
        if (dicaAdd) {
            let texto = "Os adicionais fazem parte do Diamond. Marque os que fazem sentido para você.";
            if (p.value === "diamond") {
                texto = `O Diamond inclui ${maxAdd} funcionalidades adicionais (${nAdd} escolhidas).`;
            } else if (p.value === "gold") {
                texto = "Adicionais não fazem parte do Gold. Se marcar alguma, ela entra como extra, com valor combinado na proposta.";
            }
            dicaAdd.textContent = texto;
        }
    }
    ...
    function atualizarResumo() {
        ...
        const blocoSite = $('[data-bloco="site"]');
        if (blocoSite && !blocoSite.hidden) {
            const n = marcados("basicas").length + marcados("adicionais").length;
            linhas.push(["Funcionalidades do site", String(n)]);
        }

        const blocoMkt = $('[data-bloco="marketing"]');
        if (blocoMkt && !blocoMkt.hidden) {
    ...
    /* ---------- transição entre a tela de seleção e o resto do pedido ---------- */
    function mostrarAreaFormulario() {
        blocoSelecao.hidden = true;
        areaFormulario.hidden = false;
        rolar(areaFormulario);
    }

    function voltarParaSelecao() {
        areaFormulario.hidden = true;
        blocoSelecao.hidden = false;
        rolar(blocoSelecao);
    }

    /* ---------- ao escolher um plano ---------- */
    function aplicarEscolhaPlano() {
        const p = planoAtual();
        if (!p) return;

        painel.dataset.plano = p.value;
        $("#plano-escolhido-nome").textContent = p.dataset.nome;

        if (p.value === "indefinido") {
            // ainda falta escolher "site / marketing / os dois" antes de liberar o resto
            tipoIndefinidoDiv.hidden = false;
            return;
        }

        tipoIndefinidoDiv.hidden = true;
    ...
    $("#trocar-plano").addEventListener("click", voltarParaSelecao);
    ...
    form.addEventListener("submit", (e) => {
        ...
        $$(".bloco, .envio", painel).forEach((el) => (el.hidden = true));
    ...
    $("#novo-pedido").addEventListener("click", () => {
        ...
        sucesso.hidden = true;
        $$(".bloco, .envio", painel).forEach((el) => (el.hidden = false));
        voltarParaSelecao();
    });
